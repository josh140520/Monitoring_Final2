...
class GraphWindow(Screen): #3rd window
    ...
    def show_temp(self, instance):
        ...
        with self.ids.temp_layout.canvas:
            ...
            # Replace None values with 0
            listY = [0 if y is None else y for y in listY]
            position_offset = (self.ids.temp_layout.width * 0.335+0.0139, self.ids.temp_layout.height * 0.25)

            # Create a list of points by interleaving x and y coordinates
            points = []
            for x, y in zip(listX, listY):
                points.extend([
                    self.ids.temp_layout.x + position_offset[0] + self.ids.temp_layout.width * (x / max(listX)) * 0.334*1.67,
